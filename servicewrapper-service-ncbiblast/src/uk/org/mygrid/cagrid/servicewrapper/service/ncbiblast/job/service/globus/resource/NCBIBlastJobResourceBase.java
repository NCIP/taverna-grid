...
import uk.org.mygrid.cagrid.servicewrapper.service.ncbiblast.service.NCBIBlastConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class NCBIBlastJobResourceBase extends ReflectionResource implements Resource
                                                  ,PersistenceCallback
                                                  ,TopicListAccessor
                                                  ,RemoveCallback
                                                  {

	static final Log logger = LogFactory.getLog(NCBIBlastJobResourceBase.class);

	private NCBIBlastJobResourceConfiguration configuration;
    ...
    public NCBIBlastJobResourceBase() {
        try {
            resourcePropertyPersistenceHelper = new gov.nih.nci.cagrid.introduce.servicetools.XmlPersistenceHelper(NCBIBlastJobResourceProperties.class,NCBIBlastConfiguration.getConfiguration());
            resourcePersistenceHelper = new FilePersistenceHelper(this.getClass(),NCBIBlastConfiguration.getConfiguration(),".resource");
        } catch (Exception ex) {
	...
	public void setTerminationTime(Calendar time) {
		...
		Topic terminationTopic = ((Topic)getResourcePropertySet().get(NCBIBlastJobConstants.TERMINATIONTIME));
	...
	public gov.nih.nci.cagrid.metadata.service.Fault getFault(){
		return ((NCBIBlastJobResourceProperties) getResourceBean()).getFault();
	}
	
	public void setFault(gov.nih.nci.cagrid.metadata.service.Fault fault ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(NCBIBlastJobConstants.FAULT);
		prop.set(0, fault);
        //call the first store to persist the resource
        store();
	}
	...
	public NCBIBlastJobResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (NCBIBlastJobResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
    ...
    public void load(ResourceKey resourceKey) throws ResourceException, NoSuchResourceException, InvalidResourceKeyException {
	  beingLoaded = true;
	   ...
	   NCBIBlastJobResourceProperties props = (NCBIBlastJobResourceProperties)resourcePropertyPersistenceHelper.load(NCBIBlastJobResourceProperties.class, resourceKey.getValue());
       this.initialize(props, NCBIBlastJobConstants.RESOURCE_PROPERTY_SET, resourceKey.getValue());
