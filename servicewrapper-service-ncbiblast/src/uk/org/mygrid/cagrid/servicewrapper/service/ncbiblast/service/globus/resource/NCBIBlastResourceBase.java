...
import uk.org.mygrid.cagrid.servicewrapper.service.ncbiblast.common.NCBIBlastConstants;
import uk.org.mygrid.cagrid.servicewrapper.service.ncbiblast.stubs.NCBIBlastResourceProperties;
 ...
 * 
 */
public abstract class NCBIBlastResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(NCBIBlastResourceBase.class);

	private NCBIBlastResourceConfiguration configuration;
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((NCBIBlastResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(NCBIBlastConstants.SERVICEMETADATA);
	...
	public NCBIBlastResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (NCBIBlastResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((NCBIBlastResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
