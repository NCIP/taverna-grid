package uk.org.mygrid.cagrid.servicewrapper.service.interproscan.service.globus.resource;
...
import uk.org.mygrid.cagrid.servicewrapper.service.interproscan.common.InterProScanConstants;
import uk.org.mygrid.cagrid.servicewrapper.service.interproscan.stubs.InterProScanResourceProperties;
 ...
 * 
 */
public abstract class InterProScanResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(InterProScanResourceBase.class);

	private InterProScanResourceConfiguration configuration;
    ...
    public InterProScanResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((InterProScanResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(InterProScanConstants.SERVICEMETADATA);
	...
	public InterProScanResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (InterProScanResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((InterProScanResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
