...
public class InterProScanImpl extends InterProScanImplBase {
  ...
  public uk.org.mygrid.cagrid.servicewrapper.service.interproscan.job.stubs.types.InterProScanJobReference interProScan(uk.org.mygrid.cagrid.domain.interproscan.InterProScanInput interProScanInput) throws RemoteException {
		...
		// TODO: Submit job
	  	...
	  	InterProScanInputParameters origParams = interProScanInput.getInterProScanInputParameters();
	  	...
	  	InputParams params = uk.ac.ebi.www.wsinterproscan.InputParams.Factory.newInstance();
	  	if (params == null) {
	  	...
	  	if (origParams.getSignatureMethod() != null) {
		  	for (SignatureMethod signatureMethod : origParams.getSignatureMethod()) {
			  	if (apps.length() > 0) {
			  		apps.append(' ');
			  	}
	  	...
	  	SequenceRepresentation seqRep = interProScanInput.getSequenceRepresentation();
	  	if (seqRep == null) {
	  	...
	  	if (seqRep instanceof ProteinSequenceRepresentation) {
	  		...
	  		throw new RemoteException("Unsupported sequence representation type " + seqRep.getClass());
	  	}
		...
		if (seqRep instanceof FASTANucleotideSequence) {
			content[0].setContent(((FASTANucleotideSequence) seqRep).getSequence());
			content[0].setType("sequence");
		} else if (seqRep instanceof FASTAProteinSequence) {
			content[0].setContent(((FASTAProteinSequence) seqRep).getSequence());	
			content[0].setType("sequence");
		} else if (seqRep instanceof ProteinSequenceIdentifier) {
			content[0].setContent(((ProteinSequenceIdentifier) seqRep).getSequenceId());	
			content[0].setType("sequence"); // oddly enough..
		} else if (seqRep instanceof NucleotideSequenceIdentifier) {
			content[0].setContent(((NucleotideSequenceIdentifier) seqRep).getSequenceId());	
			content[0].setType("sequence"); // oddly enough..
		} else {			
			throw new RemoteException("Unsupported sequence representation type " + seqRep.getClass());
		}	
		...
		try {
			...
			final InterProScanJobResource resource = resourceHome.getResource(resourceKey);
