...
public class CaGridActivityHealthChecker implements HealthChecker<CaGridActivity> {

	public boolean canHandle(Object subject) {
		return subject != null && subject instanceof CaGridActivity;
	}

	public HealthReport checkHealth(CaGridActivity activity) {
		...
		try {
			...
			reports.add(new HealthReport("CaGrid Activity","Error whilst parsing the WSDL:"+e.getMessage(),Status.SEVERE));
		} catch (WSDLException e) {
			reports.add(new HealthReport("CaGrid Activity","Error whilst parsing the WSDL:"+e.getMessage(),Status.SEVERE));
		} catch (IOException e) {
			reports.add(new HealthReport("CaGrid Activity","Communication error whilst parsing the WSDL:"+e.getMessage(),Status.SEVERE));
		} catch (SAXException e) {
			reports.add(new HealthReport("CaGrid Activity","XML error whilst parsing the WSDL:"+e.getMessage(),Status.SEVERE));
		}

		Status status = highestStatus(reports);
		HealthReport report = new HealthReport("CaGrid Activity", "",
				status, reports);
