 ...
 ******************************************************************************/
package net.sf.taverna.cagrid.wsdl.parser;
...
public class WSDLParser {
	...
	private ArrayTypeDescriptor constructMapType(TypeEntry type) {
		...
		// Method getItemQName() not supported in axis 1.2 used by globus, so replaced with the line below
		//TypeEntry mapItem = getSymbolTable().getType(type.getItemQName());
		TypeEntry mapItem = getSymbolTable().getType(type.getQName());
	...
	private List constructElements(List elements) {
		...
		for (Iterator iterator = elements.iterator(); iterator.hasNext();) {
			...
			// Method getMaxOccursIsUnbounded() not supported in axis 1.2 used by globus
	...
	private ArrayTypeDescriptor constructArrayType(TypeEntry type) {
		...
		result.setType(type.getQName().getLocalPart());
		result.setQname(type.getQName());
		// Method getItemQName() not supported in axis 1.2 used by globus
